...
def upload_to_hf(local_model_path: str, hf_repo_id: str, hf_token: str):
    ...
    if not hf_repo_id:
        raise ValueError("HF_REPO_ID is not set. Cannot upload to Hugging Face.")

    api = HfApi()
    # Validate token early so CI errors are explicit in logs.
    api.whoami(token=hf_token)

    # Create the repo if it doesn't exist (under your user or org)
    create_repo(repo_id=hf_repo_id, token=hf_token, private=False, exist_ok=True,repo_type="model",)
    ...
    upload_file(
        ...
        token=hf_token,
        repo_type="model",
        commit_message="Upload model.joblib from GitHub Actions",
    )
...
if __name__ == "__main__":
    ...
    # In CI, fail loudly if required secrets are missing.
    if os.getenv("GITHUB_ACTIONS") == "true" and (not hf_repo_id or not hf_token):
        raise ValueError("Missing HF_REPO_ID or HF_TOKEN secret in GitHub Actions")
